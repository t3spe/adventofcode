...
sleepyhead = t[:minutes].keys.collect { |k| [t[:minutes][k].values.sum, k] }.sort.reverse.first[1]
puts sleepyhead

max_minutes = t[:minutes][sleepyhead].values.max
kv_max_minutes = t[:minutes][sleepyhead].select { |k, v| v.eql?(max_minutes) }.keys
puts kv_max_minutes.inspect

puts "-" * 80
puts sleepyhead * kv_max_minutes[0]
