...
File.readlines("input2.txt").collect(&:chomp).reject(&:empty?).each do |line|
  line.split("").each_with_index do |e, x|
    g[[x, y, z, w]] = 1 if e.eql?("#")
  end
  y += 1
end

# delta by dimension for neighbours
neg = (-1..1).to_a.product((-1..1).to_a).product((-1..1).to_a).product((-1..1).to_a).collect { |e| e.flatten } - [[0, 0, 0, 0]]
...
def update_grid(g, neg)
  ng = {}
  x0, x1, y0, y1, z0, z1, w0, w1 = compute_bounds(g)
  x0.upto(x1).each do |x|
    y0.upto(y1).each do |y|
      z0.upto(z1).each do |z|
        w0.upto(w1).each do |w|
          curr = 0
          curr = g[[x, y, z, w]] if g.key?([x, y, z, w])
          # now see how many neg we have
          active_neg = neg.collect do |n|
            nc = [x + n[0], y + n[1], z + n[2], w + n[3]]
          ...
          when 0
            ng[[x, y, z, w]] = 0
            ng[[x, y, z, w]] = 1 if active_neg.eql?(3)
          when 1
            ng[[x, y, z, w]] = 0
            ng[[x, y, z, w]] = 1 if active_neg.eql?(2) || active_neg.eql?(3)
          end
        end
      end
    end
