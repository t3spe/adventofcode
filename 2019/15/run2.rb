...
b.empty_spaces.each do |sp|
  puts "-" * 9
  puts "inspecting #{sp}"
  acc = []
  b.origin_to_oxygen(sp, [], acc)
  puts acc.inspect
  takes_minutes = acc.collect { |a| a.size - 1 }.min
  next if takes_minutes.nil?
  max_takes_minutes = [takes_minutes, max_takes_minutes].max
end

puts "=" * 80
puts max_takes_minutes
