...
def compute_dir_sizes(r, acc = [0], need = 100000)
  size = 0
  r.children.each do |k,v|
    if v.type.eql?(:f)
      size += v.size
    else
      size += compute_dir_sizes(v, acc, need)
    end
  end
  r.size = size
  acc[0] = [acc[0], size].min if size >= need
  return size
end

compute_dir_sizes(r)
acc = [r.size]
compute_dir_sizes(r, acc, 30000000 - (70000000 - r.size))
